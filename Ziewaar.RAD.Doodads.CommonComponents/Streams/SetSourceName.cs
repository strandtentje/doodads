...
public class SetSourceName : IService
{
    ...
    [EventOccasion(
        "Likely happens when there was no name or actual sourcing interaction")]
    public event CallForInteraction? OnException;

    public void Enter(StampedMap constants, IInteraction interaction)
    {
        if ((constants, SourceNameConstant).IsRereadRequired(
                out string? sourceNameCandidate))
            this.CurrentSourceName = sourceNameCandidate;
        if (this.CurrentSourceName == null || string.IsNullOrEmpty(this.CurrentSourceName))
        {
            OnException?.Invoke(this,
                new CommonInteraction(interaction,
                    "source name required as primary constant"));
            return;
        }
            ...
            sourcingInteraction == null)
        {
            OnException?.Invoke(this,
                new CommonInteraction(interaction,
                    "sourcing interaction required"));
            return;
        }

        OnThen?.Invoke(this,
            new SourceNamingInteraction(interaction, this.CurrentSourceName,
                sourcingInteraction));
    }
